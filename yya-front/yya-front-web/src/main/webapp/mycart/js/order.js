...
//		});
   ...
   error : function(XMLHttpResponse) {
	...
	function submit_Order() {
		$("#submit_message").hide();
		var inputHidden = $("input:checked").parent().parent().find("input:hidden");
		if (inputHidden.val() == "" || inputHidden.val() == undefined || inputHidden.val() == null){
            layer.alert("请选择收货地址");
            return false;
		}
		$("#orderForm").append(inputHidden);
		jQuery.ajax( {
			type : "POST",
			dataType : "json",
